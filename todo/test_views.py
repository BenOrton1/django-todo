...
class TestViews(TestCase):
    ...
    def test_post_create_an_item(self):
        response = self.client.post("/add", {"name":"crrate a test"})
    ...
    def test_post_edit_an_item(self):
        ...
        response = self.client.post("/edit/{0}".format(id),{"name":"A differnt name"})
        item = get_object_or_404(Item, pk=id)
        self.assertEqual("A differnt name", item.name)
